...
\pdfinfo{
/Title (Monte-Carlo Tree Search in Parameterized Poker Squares)
...
%
\title{Monte-Carlo Tree Search in Parameterized Poker Squares}    % ***** DON'T FORGET TO PUT TITLE IN THE METADATA ABOVE TOO!
\author{Steven Bogaerts, Robert Arrington, Clay Langley\\
...
\addplot [only marks, mark size=1pt] table[y=mean_value, x=cp]{cpexperiment.dat};
\end{axis}
\end{tikzpicture}
\end{center}
...
{\bf Added by Clay} Many papers about MCTS claim that adding specified domain knowledge into their player greatly increased performance, no matter the game being played. In our PPS player, we implemented two types of domain knowledge.
...
{\bf Added by Clay} After the effectiveness of our board evaluator at improving our player’s average score, we decided to apply a similar technique to the expansion step as well. Similar to evaluating possible moves during the simulation step, we used our algorithm to calculate estimated board values of moves that could be made in the actual game, then only created nodes for the moves that would result in a score above a specified threshold, effectively pruning the rest. This threshold was based on a percentage of the parent of the moves being considered’s score. In order to avoid overpruning, we implemented a minimum percentage of children that could be pruned. If the threshold algorithm pruned enough children that the percentage of children remaining went below our minimum, we would add back in any children above the average estimated values of all the children. We found that this method significantly reduced the size of the game space, but only resulted in a relatively minor increase in score when compared to the increase given by our improvement to the simulation step.
