...
%while much of the MCTS research has been applied to multi-player games. Often these games can be played via a form of minimax.
%Prior to MCTS these typically were attempted to be solved with a form of minimax for zero-sum games which end in a win, lose, or draw. %For single-player games the strategy is significantly different where a cumulative best score derived from a game's scoring rules %determine a better or worse outcome, not a win or loss, but simply a higher or lower final score. {\bf SAB hmm, not entirely clear in my %opinion} }

%single-player
 %\cite{schadd2012single} describe the use of these two kinds of nodes as well as the swapping of the simulation and expansion steps in %order to have a more informed expansion process.
%reduce the amount randomness 
%We did not follow the MCTS variation as proposed by \cite{schadd2012single} which switches the position of the simulation and %expansion steps for the game {\it SameGame}. 
%selection simulation expansion backpropagation

%However, we did find the application of a heuristic to the simulation a notable technique which we did employ. \cite{schadd2012single} %did so to remove some of the randomness from what would be our chance node selection.
...
\begin{table}
\captionof{table}{Selection and simulation results over 2000 games}
\label{tbl:SelSim}
\centering
\begin{tabular}{c c c c}
\hline
...
%*****Why does best-move simulation show a much larger gain than prune-UCT selection? One possible explanation is that for each single application of the heuristic in selection/expansion, there are many applications of the heuristic in simulation. So simulation comes at a greater cost (compare deficit rows 3,1 vs. 2,1) but is also used more frequently, thus the domain knowledge of the heuristic is applied much more frequently in simulation.
