...
public class xPokerSquares {
	...
	public static final long GAME_MILLIS = 30000L; // EAAI-2016 contest maximum milliseconds per game is 30000
	 ...
	 */
	public xPokerSquares(PokerSquaresPlayer player, PokerSquaresPointSystem system) {
		this.player = player;
		this.system = system;
	 ...
	 */
	public int[] playSequence(long numGames, long startSeed, boolean verbose) {
		this.verbose = verbose;
		if (verbose) {
			System.out.printf("%d games starting at seed %d\nPoint system:\n%s\n", numGames, startSeed, system);
		}
		int[] scores = new int[0];
//		int[] scores = new int[(int)numGames];
//		double scoreMean = 0;
	...
	public static double[] playTournament(
		...
		for (PokerSquaresPointSystem system : systems) { // for each point system
			...
			for (int i = 0; i < players.size(); i++) { // for each player
				...
				int[] scores = new xPokerSquares(player, system).playSequence(gamesPerSystem, startSeed, false);
...
//		new PokerSquares(new xRandomRolloutSquash1000MaxPlayer1rt2(), PokerSquaresPointSystem.getRandomPointSystem()).play();
//
...
//		new PokerSquares(new GreedyMCPlayer(2), PokerSquaresPointSystem.getAmeritishPointSystem()).playSequence(3, 0, false);
		new xPokerSquares(new RandomPlayer(), PokerSquaresPointSystem.getBritishPointSystem()).playSequence(100000000000L, 0, false);
//		// print the totals by hand
		int totalHands = 0;
		for (int i=0; i < 10; i++){
			totalHands += system.hands[i];
			System.out.println("hand: " + i + "\t" + system.hands[i]);
		}
...
//		PokerSquares.playTournament(players, systems, 50, 0L); // use fewer games per system for faster testing
	}
}
